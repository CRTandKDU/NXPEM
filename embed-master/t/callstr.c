...
static int cb_nxpstr(vm_extension_t * const v) {
  /* char   *str; */
  char   buf[] = "foo";
  int    r;
  cell_t cell;
  ...
  short len = 3;
  r = embed_pop( v->h, &cell );
  if( 0 == r ){
    for( short i=0; i<len; i += 2 ){
      cell_t addr = (cell + i)>>1%L;
      cell_t val  = buf[i] + ( (((i+1) > len) ? 0 : buf[i+1])<<8 );
      fprintf( stderr, "Writing %d at %d\n", val, addr );
      mw( v->h, addr, val );
      fprintf( stderr, "Wrote   %d at %d\n", mr( v->h, addr ), addr );
    }
  }
  return 0;
}
